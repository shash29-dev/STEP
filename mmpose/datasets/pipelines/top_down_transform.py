...
class TopDownRandomFlip:
    ...
    def __call__(self, results):
        ...
        gmsp_joints_3d = results['gmsp_kps_joints']
        gmsp_joints_3d_visible = results['gmsp_kps_joints_visible']
        ...
        if np.random.rand() <= self.flip_prob:
            ...
            if not isinstance(img, list):
                ...
                for idx,(gmspj,gmspjv)in enumerate(zip(gmsp_joints_3d,gmsp_joints_3d_visible)):
                    gmsp_joints_3d[idx], gmsp_joints_3d_visible[idx] = fliplr_joints(gmspj, gmspjv, img.shape[1],results['ann_info']['flip_pairs'])
                center[0] = img.shape[1] - center[0] - 1
                results['bbox_tfm'] = mut.flip_bbox_horizontal(results['bbox_tfm'].reshape(-1),img.shape[1])
            else:
                joints_3d, joints_3d_visible = fliplr_joints(
                    joints_3d, joints_3d_visible, img[0].shape[1],
                    results['ann_info']['flip_pairs'])
                center[0] = img[0].shape[1] - center[0] - 1
                raise('Make Flip for gmsp_kps, correct')
        ...
        results['bbox_tfm'] = results['bbox_tfm'].reshape(-1,2)
        results['gmsp_kps_joints'] = gmsp_joints_3d
        results['gmsp_kps_joints_visible'] = gmsp_joints_3d_visible
        return results
...
class TopDownAffine:
    ...
    def __init__(self, use_udp=False,validation=False):
        ...
        if self.use_udp:
            ...
            if not isinstance(img, list):
                if not self.validation:
                    # bops = [cv2.BORDER_CONSTANT, cv2.BORDER_REPLICATE, cv2.BORDER_REFLECT, cv2.BORDER_WRAP, cv2.BORDER_REFLECT_101]
                    bops = [cv2.BORDER_CONSTANT]
                    border = random.choice(bops)
                else:
                    border= cv2.BORDER_CONSTANT
                img = cv2.warpAffine(img,trans, (int(image_size[0]), int(image_size[1])),flags=cv2.INTER_LINEAR,borderMode=border)
            for i in range(results['ann_info']['num_joints']):
                if joints_3d_visible[i].sum() > 0.0:
                    joints_3d[i, 0:2] = affine_transform(joints_3d[i, 0:2], trans)

            for idxgmsp in range(gmsp_joints_3d.shape[0]):
                for i in range(results['ann_info']['num_joints']):
                    if gmsp_joints_3d_visible[idxgmsp][i].sum()>0.0:
                        gmsp_joints_3d[idxgmsp,i, 0:2] = affine_transform(gmsp_joints_3d[idxgmsp,i, 0:2], trans)
            # for idx,pt in enumerate(results['bbox_tfm']):
            #     results['bbox_tfm'][idx] = affine_transform(pt, trans)
            
            results['bbox_tfm'] = mut.transform_bbox_affine_matrix(results['bbox_tfm'].flatten().tolist(),trans)
        
        results['img'] = img
        results['bbox_tfm'] = results['bbox_tfm'].flatten().tolist()
        # gmsp_joints_3d[:,:,-1] = gmsp_joints_3d_visible.max(axis=-1)
        # xxb = imshow_bboxes(img,np.array(results['bbox_tfm'])[None],colors='red',thickness=2,font_scale=0.5,show=False)
        # xx = imshow_keypoints(img,gmsp_joints_3d,kpt_score_thr=0.3,pose_kpt_color=(np.random.rand(17,3)*255).astype(np.uint8))
        # plt.imsave('tmp.png',np.hstack((xx,xxb,img)))
        # pdb.set_trace()
        results['joints_3d'] = joints_3d
        results['joints_3d_visible'] = joints_3d_visible
        results['joints_3d'] = joints_3d
...
class TopDownGenerateTarget:
    ...
    def __call__(self, results):
        ...
        all_target= []
        all_target_weights = []
        for joints_3d,joints_3d_visible in zip(gmsp_joints_3d,gmsp_joints_3d_visible):
            if self.encoding == 'MSRA':
                target, target_weight = self._msra_generate_target(
                    results['ann_info'], joints_3d, joints_3d_visible,
                    self.sigma)
            ...
            all_target_weights.append(target_weight)

        all_target = np.stack(all_target,axis=0).mean(axis=0)
        all_target_weights=np.stack(all_target_weights,axis=0).max(axis=0)
