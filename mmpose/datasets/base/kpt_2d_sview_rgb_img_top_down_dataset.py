...
class Kpt2dSviewRgbImgTopDownDataset(Dataset, metaclass=ABCMeta):
    ...
    @staticmethod
    def _get_mapping_id_name(imgs, aif=None):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        # txt = os.path.dirname(self.ann_file)+ os.sep+os.sep.join(results['image_file'].split(os.path.dirname(self.ann_file))[-1].split('\\')[2:])
        # txtclean = "".join([x for x in txt if x.isascii()])
        # results['image_file'] = txtclean
