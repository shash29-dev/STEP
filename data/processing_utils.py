...
from data.tensorlist import TensorList
import pdb
...
def gaussian_label_function(target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
    ...
    center = feat_sz * target_center_norm + 0.5 * torch.Tensor([(kernel_sz[0] + 1) % 2, (kernel_sz[1] + 1) % 2]).to(target_bb.device)
...
def gaussian_label_function_given_cxy(cxy, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
                            uni_bias=0):
    """Construct Gaussian label function."""
    ...
    image_sz = torch.Tensor(image_sz).to(cxy.device)
    feat_sz = torch.Tensor(feat_sz).to(cxy.device)
    target_center = cxy
    target_center_norm = (target_center - image_sz / 2) / image_sz

    center = feat_sz * target_center_norm + 0.5 * torch.Tensor([(kernel_sz[0] + 1) % 2, (kernel_sz[1] + 1) % 2]).to(cxy.device)
