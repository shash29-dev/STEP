...
def get_frames_from_video(video_path,cfg,pkpts=False,numframes=20):
    ...
    while True:
        ...
        new_width = width
        new_height = height
        new_width = int(width / .75)
        new_height = int(height / .75)
        frame = cv2.resize(frame, (new_width, new_height))
        frame = frame[:,:,::-1]
        if curframe>start_idx and curframe%skip_frames==0:
            allframes.append(frame)
        if len(allframes)>400:
            break
        curframe += 1
...
class LTREvaluator():
    ...
    def generate_results_video(self,datain,pipeline,test_pipeline,ann_info,cfd,label_params,dataset_info,device,padding):
        ...
        for idx,tim in enumerate(datain['test_images']):
            ...
            if data.get('train_kpts',None) is not None and addframe:
                datatrain,_ = mut.get_processed_traininfo(datain['train_images'],datain['train_bbox'],pipeline,ann_info,cfd,label_params,dataset_info,padding=3)
    ...
    def generate_results(self,data, animid=0,metas=None,vd=None):
        torch.set_grad_enabled(self.training)
        all_im =[]
