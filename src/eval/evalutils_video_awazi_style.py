...
class LTREvaluator():
    ...
    def generate_results_video(self,datain,pipeline,test_pipeline,ann_info,cfd,label_params,dataset_info,device,padding,video_id,track_id):
        ...
        for idx,tim in enumerate(datain['test_images']):
            ...
            # Process: Stage 2
            # data= {}
            datatrain,vd = mut.get_processed_traininfo(datain['train_images'],datain['train_bbox'],pipeline,ann_info,cfd,label_params,dataset_info,padding=1.5)
            data['train_images'] = datatrain['train_images'][:,None]
            data['train_bbox'] = datatrain['train_bbox'][:,None]
            data['train_label'] = datatrain['train_label'][:,None]
            data['train_ltrb_target'] = datatrain['train_ltrb_target'][:,None]
            ...
            result= {}
            xyxy = mut.xywh_to_xyxy(bboxorim[0])
            addframe=False
            if idx<3: addframe=True
            if idx>3: addframe= adfl.append(addframe)
            if addframe:
                print('Added Frame')
                ...
                slide = True
                if slide:
                    # datain['train_images'] =   datain['train_images'][:1] + datain['train_images'][-num_train_frames-1:]
                    # datain['train_bbox'] =     datain['train_bbox'][:1] + datain['train_bbox'][-num_train_frames-1:]
                    datain['train_images'] =   datain['train_images'][-num_train_frames-1:]
                    datain['train_bbox'] =     datain['train_bbox'][-num_train_frames-1:]  
                else:
            ...
            if has_n_consecutive_false:
                ...
                break
            if idx>1:
                prev_box = fresult[-1]['orim_bbox']
                prev_box = np.array([prev_box[0],prev_box[1],prev_box[2]-prev_box[0],prev_box[3]-prev_box[1]])
                iou = self.calculate_iou(prev_box,bboxorim[0].tolist())
                distance = self.calculate_distance_between_centers(prev_box,bboxorim[0])
                # if iou<0.01:
                if iou<=0.0001 and distance>(prev_box[2])*4.2:
                    print('Pruned Tracking... {}/{} Very Less IoU'.format(iou,distance))
                    break
            result['orim_bbox']= xyxy.tolist()
            result['orim_kpts']=kptsorim.flatten().tolist()
            result['orim_bbox_gt'] = xyxy.tolist()
            result['orim_kpts_gt']=kptsorim.flatten().tolist()
            result['image_file'] = datain['test_impath'][idx]
    ...
    def visualize_all_or(self,scores,bbop,kpop, target_kpts_scores,data, imname,metas):
        ...
        dictionary_result = {
            ...
            'orim_bbox_gt': bboxes_orim,
            'orim_kpts_gt': orim_kpts,
        }
