...
class LBHinge(nn.Module):
    ...
    def forward(self, prediction, label, target_bb=None, weight=None):
        ...
        if weight is None:    
            ...
            mse_loss = F.mse_loss(prediction, positive_mask * label,reduction='none')
            weighte_mse = mse_loss * weight[:,:,0,None,None]
            loss = torch.mean(weighte_mse)
