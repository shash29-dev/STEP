...
import src.models.target_classifier.features as clf_features
import src.models.backbone as backbones
from src.model_constructor import model_constructor

import src.models.transformer.transformer as trans
import src.models.transformer.filter_predictor as fp
import src.models.transformer.heads as heads
import pdb
...
class ToMPnet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, *args, **kwargs):
        ...
        # Run head module
        test_scores, bbox_preds, kp_preds, target_kpts_scores, rdict = self.head(train_feat_head, test_feat_head, train_bb, *args, **kwargs)

        return test_scores, bbox_preds, kp_preds, target_kpts_scores, rdict
...
def tompnet101(filter_size=1, head_layer='layer3', backbone_pretrained=True, head_feat_blocks=0, head_feat_norm=True,
    ...
    filter_predictor = fp.FilterPredictor(transformer, feature_sz=feature_sz,
                                          ...
                                          use_kpts_enc=cfg.use_kpts_enc,
                                          cfg=cfg)

    classifier = heads.LinearFilterClassifier(num_channels=out_feature_dim)
    
    classifier_kpt = heads.LinearFilterClassifier_KPT(num_channels=out_feature_dim, num_kpts=num_kpts)
    bb_regressor = heads.DenseBoxRegressor(num_channels=out_feature_dim)
    kpt_regressor = heads.KPTRegressor(num_channels=out_feature_dim,feature_sz=feature_sz,num_kpts=num_kpts)
    head = heads.Head(filter_predictor=filter_predictor, feature_extractor=head_feature_extractor,
                      classifier=classifier, classifier_kpt=classifier_kpt, 
                      bb_regressor=bb_regressor, kpt_regressor=kpt_regressor,
                      separate_filters_for_cls_and_bbreg=separate_filters_for_cls_and_bbreg,
                      cfg=cfg)
